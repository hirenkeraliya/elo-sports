...
class ProfileController extends Controller
{
    public function index($open_popup=0)
    {
        ...
        $deposits = WalletTransaction::where('user_id', $id)->where('comment', 'Deposite')->orderBy('created_at', 'desc')->limit(5)->get();
        $withdrawls = WalletTransaction::where('user_id', $id)->where('comment', 'withdrawal')->orderBy('created_at', 'desc')->limit(5)->get();
