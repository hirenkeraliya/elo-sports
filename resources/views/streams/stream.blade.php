...
<div class="row">
    <div class="col-md-12">
        <div class="video">
            ...
            <h3 class="text-light">{{$data['data'][0]['user_login']}}'s stream </h3>
            @if($l_name !='')
            <p class="text-light">Game Label : {{$label_name}}</p>
            @endif
...
<div class="row">
    <div class="col-md-12">
        <div class="row">
            ...
            <div class="col-md-4">
                @if($email >0 && $email !='')
                @php $elo_bal = \App\Models\Users::select('Elo_balance')->where('id',auth()->user()->id)->first(); @endphp
                <span class="text-light">Elo Balance : {{preg_replace('#[^\w()/.%\-&]#','',$elo_bal->Elo_balance)}}</span>
                &nbsp &nbsp &nbsp
                <hr>
                @endif
                @if($email !='')
...
<div style="display:block;padding-bottom:100px;">
    ...
    @endif
    {{-- bet now modal --}}
    <div class="modal fade" id="exampleModalCentercreate_bet" tabindex="-1" role="dialog"
             aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
			 <form method="{{url('/save-new_main_bet')}}" id="create_bet_frm">@csrf
			    @method('POST')
